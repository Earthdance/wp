...
class EPL_util {
    ...
    function create_element( $args = array(), $response_view = 0 ) {
        ...
        switch ( $input_type )
        {
            ...
            case 'submit':
                ...
                $data['field'] = "<input type=\"$input_type\" id=\"{$id}\" name=\"{$name}\" class=\"$class\" rel=\"$rel\" placeholder=\"{$placeholder}\" style=\"{$style}\" {$size} value=\"$value\" $readonly $data_id /> \n";
                $data['value'] = $value;
            ...
            case 'checkbox':
                ...
                if ( is_array( $options ) && !empty( $options ) ) {
                    ...
                    foreach ( $options as $k => $v ) {
                        ...
                        $data['field'] .= "<input type=\"checkbox\" id=\"{$id}\" name=\"{$_name}{$second_key}\"  class=\"$class\"  style='{$style}' value=\"$k\" $checked $readonly $data_id /> $v \n" . $display_inline;
                        $data['table_checkbox'][$k]['f'] = "<input type=\"checkbox\" id=\"{$id}\" name=\"{$_name}{$second_key}\"  class=\"$class\"  style='{$style}' value=\"$k\" $checked $readonly $data_id /> \n";
                        $data['table_checkbox'][$k]['l'] = "$v \n";
                    }
                }
                else {
                    $checked = (isset( $k ) && $value == '' && (in_array( $k, ( array ) $value, true ))) ? 'checked = "checked"' : '';
                    $data['field'] .= "<input type=\"checkbox\" id=\"{$name}\" name=\"{$name}\"  class=\"$class\"  style='{$style}' value=\"1\" $checked $data_id /> \n" . $display_inline;
                    $data['overview'] .= " $value  \n" . $display_inline;
                }
            ...
            case 'radio':
                ...
                if ( is_array( $options ) && !empty( $options ) ) {
                    ...
                    foreach ( $options as $k => $v ) {
                        ...
                        $data['field'] .= "<input type=\"{$input_type}\" id=\"{$id}\" name=\"{$name}\"  class=\"$class\"  style='{$style}' value=\"$k\" $checked $readonly $data_id /> $v  \n" . $display_inline;
                    }
                }
                else {
                    $checked = ((in_array( $k, ( array ) $value, true ))) ? 'checked = "checked"' : '';
                    $data['field'] .= "<input type=\"{$input_type}\" id=\"{$name}\" name=\"{$name}\"  class=\"$class\"  style='{$style}' value=\"1\" $checked $data_id /> \n" . $display_inline;
                    $data['overview'] .= " $v  \n" . $display_inline;
                }
            ...
            case 'radio-switch':
                ...
                if ( is_array( $options ) && !empty( $options ) ) {
                    ...
                    foreach ( $options as $k => $v ) {
                        ...
                        $data['field'] .= "<input type=\"radio\" id=\"{$id}\" name=\"{$name}\" value=\"$k\" $checked $readonly $data_id /><label for=\"$id\" onclick=\"\">$v</label>";
                    }
                }
            ...
            case 'select':
                ...
                if ( !epl_is_empty_array( $options ) ) {
                    ...
                    $select = "<select name = '{$name}' id = '{$id}' class='$class' style='$style' $multiple $size $readonly $data_id>";
    ...
    function set_debug_message( $param, $value ) {
        ...
        $this->debug_message[$param] = print_r( $value, true );
    }
    ...
    function get_the_regis_total_amount( $symbol = true ) {
        ...
        if ( !empty( $regis_details ) && isset( $regis_details['__epl'] ) ) {
    ...
    function get_the_event_dates_times_prices() {
        ...
        foreach ( $event_details['_epl_start_date'] as $date_key => $date ) {
            if ( epl_is_ongoing_event() || true) {
                ...
                $_r[$date_key]['date'] = array( 'disp' => epl_formatted_date( $date ) . $_end . ' ' . $_note . ' ' . $_location );
    ...
    function get_the_register_button( $event_id = null, $url_only = false, $args = array() ) {
        ...
        if ( !$locked && epl_sc_is_enabled() && empty( $args['no_modal'] ) ) {
            ...
            if ( isset( $events_in_cart[$event_id] ) ) {
                $button_text = epl__( "In The Cart (View)" );
                $class .= ' in_cart';
            }
        }
...
';
    ...
    function get_days_for_widget( $for = 0, $args = array() ) {
        ...
        if ( $event_id = epl_get_element( 'event_id', $args, null ) ) {
            ...
            if ( $exclude_past_event == false && $for > 0 )
                $WHERE = '';
        ...
        //echo "<pre class='prettyprint'>" . __LINE__ . "> " . print_r(   $q, true). "</pre>";
        $new = true;
        while ( $q->have_posts() ) {
            $q->the_post();
            ...
            if ( empty( $s ) || (epl_get_element( 'class_display_type', $widget_options, 2 ) == 1 && epl_get_element( 'class_display_type', $args, 0 ) == 0) )
...
//there is another way, coming
    ...
    function send_email( $args = array() ) {
        ...
        $from_name = html_entity_decode( epl_get_element( '_epl_email_from_name', $notif_data, get_bloginfo( 'name' ) ), ENT_QUOTES );
        $from_email = epl_get_element( '_epl_from_email', $notif_data, get_bloginfo( 'admin_email' ) );
        $subject = html_entity_decode( epl_get_element( '_epl_email_subject', $notif_data, epl__( 'Registration Confirmation' ) ), ENT_QUOTES );
...
//TODO - refactor
    function send_confirmation_email( $data ) {
        ...
        $subject = html_entity_decode( epl_get_element( '_epl_email_subject', $_notif_data, $defaults['subject'] ), ENT_QUOTES );
        ...
        if ( (isset( $customer_email )) && $customer_email != '' ) {
            @wp_mail( $customer_email, $subject, $email_body, $headers, epl_get_element( 'attachment', $data, null ) );
        }
        //admin email
