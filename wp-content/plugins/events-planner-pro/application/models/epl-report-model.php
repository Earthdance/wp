...
class EPL_report_model extends EPL_Model {
    ...
    function transactions( $where = null ) {
        ...
        $q = $wpdb->get_results(
                "SELECT
          r.*, rp.*, sum(rp.payment_amount) as payment_amount
    ...
    function get_the_email_addresses( $filter = null, $regis_id = null ) {
        ...
        $q = $wpdb->get_results(
                "SELECT
          r.regis_id, r.status, r.regis_key, rd.event_id, rf.field_id,rf.input_slug, rf.value
          ...
          WHERE 1=1 AND r.status between 2 and 5 
          {$this->WHERE} AND rf.form_no = 0 {$_filter}
          GROUP BY r.regis_id
        ...
        if ( $q ) {
            foreach ( $q as $row ) {
                ...
                $email_list['raw_list'][$row->regis_id] = $d['email'];
            }
        }
    ...
    function attendee_form_data( $filter = null, $event_id = null ) {
        ...
        $q = $wpdb->get_results(
          ...
          ON r.regis_id = rp.regis_id
          WHERE 1=1 AND (r.status >= 2 AND r.status <= 5)
          {$this->WHERE} {$filter}
          GROUP BY rd.id
          ORDER BY  r.regis_date, rd.id
          "
    ...
    function get_attendee_counts( $event_id = null, $refresh = false, $all_statuses = false ) {
        ...
        foreach ( $registrations as $regis ) {
            ...
            if ( !$pack_regis ) {
                ...
                if ( $offset_another ) {
                    $current_att_count["_total_att_{$event_id}_price_{$regis->date_id}_{$regis->time_id}_" . $offset_another_key] = $qty;
                }

                if ( !isset( $current_att_count["_total_att_{$event_id}_date_{$regis->date_id}"] ) ) {
                    $current_att_count["_total_att_{$event_id}_date_{$regis->date_id}"] = $qty;
                    $current_att_count["_total_att_{$event_id}_time_{$regis->date_id}_{$regis->time_id}"] = $qty;
                    $current_att_count["_total_att_{$event_id}_price_{$regis->date_id}_{$regis->time_id}_{$regis->price_id}"] = $qty;
                ...
                foreach ( $event_details['_epl_start_date'] as $date_id => $date ) {
                    ...
                    (isset( $current_att_count["_total_att_{$event_id}_date_{$date_id}"] ) ) ? $current_att_count["_total_att_{$event_id}_date_{$date_id}"]+=$qty : $current_att_count["_total_att_{$event_id}_date_{$date_id}"] = $qty;
                    (isset( $current_att_count["_total_att_{$event_id}_time_{$date_id}_{$regis->time_id}"] ) ) ? $current_att_count["_total_att_{$event_id}_time_{$date_id}_{$regis->time_id}"]+=$qty : $current_att_count["_total_att_{$event_id}_time_{$date_id}_{$regis->time_id}"] = $qty;
                }
