...
// $this->get_event_discount_codes();
    ...
    function process_discount( $totals, $glob_disc = false ) {
        ...
        $allow_global_discounts = (epl_get_element( '_epl_allow_global_discounts', $this->discount_configs, 10 ) == 10);
        ...
        if ( $_entered_code ) {
            ...
            if ( $_code_id !== false ) {
                ...
                if ( $discounted_amount >= 0 ) {
                    ...
                    $this->totals['money_totals']['discount_description'] = $discount_description;
    ...
    function get_discount_used_count( $discount_id, $discount_code = null ) {
        ...
        if ( $all_used_from_event === false ) {
            ...
            $all_used_global = $wpdb->get_results( "SELECT r.discount_code_id, count(r.ID) as discount_used_count, r.discount_source_id
                FROM $wpdb->epl_registration as r
                INNER JOIN $wpdb->epl_regis_events re ON r.regis_id = re.regis_id
                WHERE (r.status = 2 OR r.status = 5)                
                AND NOT r.discount_code_id = '' 
        ...
        if ( $all_used_from_event || $all_used_global ) {
            ...
            if ( $this->discount_source == 'e' ) {
                ...
                $r = epl_get_element_m( 'discount_used_count', $discount_id, epl_object_to_array( $all_used_global ), 0 );
        }
...
//$this->adjust_discountable_amounts( 'category', $_categories );
    ...
    function adjust_discountable_amounts( $for = 'category', $args = null ) {
        ...
        foreach ( $event_totals as $event_id => $totals ) {
            ...
            if ( !has_term( $args, 'epl_event_categories', $event_id ) ) {
                //echo "<pre class='prettyprint'>" . __LINE__ . "> " . print_r($event_id, true). "</pre>";
                //$this->totals['money_totals']['grand_total'] -= $totals['money_totals']['grand_total'];
                //$this->totals['money_totals']['non_discount_grand_total'] += $totals['money_totals']['grand_total'];
                //s$this->totals['money_totals']['surcharge'] -= $totals['money_totals']['surcharge'];

                $this->totals['money_totals']['discountable_total'] -= $totals['money_totals']['discountable_total'];
                $this->totals['money_totals']['non_discountable_total'] += $totals['money_totals']['grand_total'];
                $this->totals['money_totals']['num_discountable_events_in_cart'] -= 1;

                //$this->totals['money_totals']['subtotal'] -= $totals['money_totals']['subtotal'];
                //$this->totals['_att_quantity']['total'] -= $totals['_att_quantity']['total'][$event_id];
                //$this->totals['_att_quantity']['total_n'] -= $totals['_att_quantity']['total_n'][$event_id];
            }
        }
