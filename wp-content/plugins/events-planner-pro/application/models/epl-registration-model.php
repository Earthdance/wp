...
class EPL_registration_model extends EPL_model {
    ...
    function refresh_data() {
        ...
        if ( !$this->on_admin ) {
            ...
            $_SESSION['__epl'] = (is_array( $_SESSION['__epl'] )) ? $_SESSION['__epl'] : array();
    ...
    function get_the_totals( $_event_id = null, $cart_grand_totals = true ) {
        ...
        foreach ( $events_in_cart as $event_id => $totals ) {
            $data['money_totals'] = $totals['money_totals'];
            $data['mode'] = $this->mode;
    ...
    function calculate_cart_totals( $refresh = true, $refresh_curr_data = true ) {
            ...
            if ( (!empty( $_REQUEST['event_id'] ) && $_REQUEST['event_id'] != $event_id) || isset( $_REQUEST['epl_m'] ) || epl_get_element( 'caller', $_REQUEST ) == 'summary' ) {
                ...
                if ( !is_null( $this->get_att_quantity_values() ) ) {
                    ...
                    foreach ( ( array ) $att_qty as $price_id => $price_qty ) {
                        ...
                        if ( epl_get_element_m( $price_id, '_epl_price_surcharge_amount', $event_details ) > 0 && $_qty > 0 ) {
                            ...
                            $data[$event_id]['money_totals']['surcharge'] += $_tmpsurcharge['surcharge'];
        ...
        epl_debug_message( 'cart totals', $cart_totals );
        epl_debug_message( 'current data', $this->current_data );
        return $data;
    }
...
//if surcharge is greater than 0
    ...
    function _get_prices( $date_id = null ) {
        ...
        if ( epl_is_empty_array( $event_details['_epl_price_name'] ) )
            return null;
...
//        $this->calculate_cart_totals();
    ...
    function get_payment_options() {
        ...
        $payment_choices = epl_get_element( '_epl_payment_choices', $event_details, array() );
        $default_selected = epl_get_element( '_epl_default_selected_payment', $event_details, null );

        if ( epl_sc_is_enabled() ) {
            $payment_choices = epl_get_setting( 'epl_sc_options', 'epl_sc_payment_choices', array() );
            $default_selected = epl_get_setting( 'epl_sc_options', 'epl_sc_default_selected_payment', array() );
        }
        ...
        foreach ( ( array ) $payment_choices as $payment_choice ) {
            ...
            $_payment = array(
                ...
                'default_checked' => (is_null( $value ) && ($default_selected == $payment_choice || is_null( $default_selected )) || count( $payment_choices ) == 1) ? 1 : 0,
                'display_inline' => 1,
                'required' => true,
                'value' => $value
            );
    ...
    function get_registration_forms( $args ) {
        global $event_totals, $event_details;
        ...
        if ( $process != 'esc' || $scope == 'waitlist' ) {
            ...
            $discount_code_id = epl_get_element_m( 'discount_code_id', 'money_totals', $event_totals );
    ...
    function construct_form( $scope, $event_id, $forms, $attendee_number, $price_name = '', $price_id = null, $date_display = null ) {
        ...
        global $event_details, $customer_email, $regis_details;
        ...
        if ( $scope == 'ticket_buyer' && !is_user_logged_in() && epl_um_is_enabled() && $this->add_new_user_enable() != 0 && $this->add_new_user_method() == 1 ) {
            ...
            $ur_specific = epl__( "This email will be used to grant you membership access to our website.  You will receive an email with your password after completing this registration." );
...
//to compensate for pre 1.2.9 data 
        ...
        if ( $scope == 'regis_forms' )
            $data['copy_link'] = (epl_get_element( '_epl_enable_form_to_form_copy', $event_details, 0 ) == 10);
            ...
            foreach ( $epl_fields_to_display as $field_id => $field_atts ) {
                ...
                if ( empty( $_val ) ) {
                    ...
                    elseif ( $ticket_number == 0 ) {
                        ...
                        $args['default_value'] = !(is_array( $def_val )) ? $def_val : '';
                    }
                    else
        ...
        if ( $scope == 'ticket_buyer' ) {
            ...
            if ( epl_get_setting( 'epl_api_option_fields', 'epl_mc_action' ) != 0 && (epl_get_element( '_epl_offer_notification_sign_up', $event_details ) == 1 && $global_newsletter_ok) ) {
                ...
                $_newsletter_signup = array(
                    ...
                    'options' => epl_yes_no(),
                    'value' => $this->current_data[$this->regis_id]['newsletter_signup'],
                    'class' => 'epl_w70'
                );
        ...
        if ( $ticket_number == 0 ) {
            ...
            $r = "<div id='epl_form_section--" . (isset( $ticket_number ) ? $event_id . '-' . $price_id . '-' . $ticket_number : 0) . "' class='epl_regis_attendee_wrapper'><fieldset class='epl_fieldset'><legend>" . $data['ticket_counter_label'] . ' ' . $ticket_number . ': ' . $data['price_name'] . " - {$event_details['post_title']}</legend> {$lookup_form} $copy_from" . $data['form'] . '</fieldset></div>';
        }
...
//move to common helper
    ...
    function add_registration_to_db( $meta = array() ) {
        ...
        //        echo "<pre class='prettyprint'>" . __LINE__ . "> " . print_r( $db_data, true ) . "</pre>";
        //exit;
        $this->current_data['post_ID'] = $regis_post_ID;
    ...
    function ok_to_proceed( $force_check = false, $event_id = null ) {
        ...
        foreach ( $event_cart_details as $event_id => $_totals ) {
            if ( $_totals == 0 )
                continue;
            $total_att = epl_get_element( $event_id, $_totals['_att_quantity']['total'] );
            ...
            if ( epl_is_empty_array( $cart_dates[$event_id] ) && epl_get_element( 'cart_action', $_REQUEST ) != 'add' ) {
    ...
    function get_the_cart_selected_payment() {
        ...
        if ( !$p && epl_regis_flow() == 10 ) {
            $p = epl_get_element( '_epl_payment_choices', $event_details, array() );
        }


        return (epl_is_empty_array( $p )) ? null : $p;
    }


    function get_payment_profile_id() {

        return epl_get_element( 0, $this->get_the_cart_selected_payment(), null );
    }
    ...
    function has_selected_cc_payment( $gateway_info = null ) {

        $gateway_info = (!is_null( $gateway_info ) ? $gateway_info : $this->get_gateway_info());

        $pay_type = epl_get_element( '_epl_pay_type', $gateway_info );

        if ( $pay_type == '_pp_pro' || $pay_type == '_auth_net_aim' || $pay_type == '_qbmc' || $pay_type == '_firstdata' || $pay_type == '_usa_epay' || $pay_type == '_pp_payflow'
        )
            return true;
