...
class EPL_Gateway_Model extends EPL_Model {
    ...
    function _express_checkout_redirect() {
        ...
        $response = $paypal->request( 'SetExpressCheckout', $requestParams + $item + $orderParams );
    ...
    function _exp_checkout_do_payment() {
        ...
        $response = $paypal->request( 'DoExpressCheckoutPayment', $requestParams + $orderParams + $item ); //ALANE - adding LINE ITEMS TO request
    ...
    function paypal_pro_process() {
        ...
        $min_deposit = epl_get_element_m( 'min_deposit', 'money_totals', $_totals, 0 );
        ...
        $orderParams = array(
            ...
            'ITEMAMT' => ($min_deposit != 0) ? epl_get_balance_due() : $subtotal,
            'TAXAMT' => $tax,
            'SHIPPINGAMT' => 0,
        ...
        $item = array(
            ...
            'L_DESC0' => $event_details['post_title'] . ', Qty: ' . $_totals['_att_quantity']['total'],
            'L_AMT0' => ($min_deposit != 0) ? epl_get_balance_due() : $subtotal,
            'L_QTY0' => '1'
        );
...
<div style="display:none">
          ...
          <input type='text' name="x_receipt_link_URL" value="$relay_url" />
              <input type='text' name="x_header_html_receipt" value="<style type='text/css' media='all'>#btnClickAway{color:red;font-weight:bold;}</style>" />
...
EOT;
    ...
    function moneris_process() {
        ...
        parse_str( str_replace( array( ' ', '<br>' ), array( '', '&' ), $post_response ), $response_array );

        $rvar_eid = epl_get_element( 'rvar_eid', $_POST );
        $rvar_pid = epl_get_element( 'rvar_pid', $_POST );
        $rvar_rid = epl_get_element( 'rvar_rid', $_POST );
        ...
        //approved
        if ( $response_array['response_code'] < 50 ) {


            $data['post_ID'] = $post_ID;

            $data['_epl_grand_total'] = number_format( epl_get_balance_due(), 2, '.', '' );
            $data['_epl_payment_amount'] = epl_get_element( 'charge_total', $_POST );
            $data['_epl_payment_date'] = current_time( 'mysql' );
            $data['_epl_payment_method'] = $this->erm->get_payment_profile_id();
            $data['_epl_transaction_id'] = epl_get_element( 'txn_num', $_POST );
            $data['_epl_prediscount_total'] = number_format( epl_get_element( 'pre_discount_total', $_totals['money_totals'], 0 ), 2, '.', '' );
            $data['_epl_discount_amount'] = number_format( epl_get_element( 'discount_amount', $_totals['money_totals'], 0 ), 2, '.', '' );
            ...
            return '<div class="epl_error">ERROR: ' . $response_array['status'] . '</div>';
        }
    }
