    ...
    if ( !empty( $payment_data ) ) {
        ...
        foreach ( $payment_data as $time => $p ) {
            if ( $p['_epl_payment_amount'] == 0 )
                continue;
            $regis_total -= $p['_epl_payment_amount'];
        }
    } else {
...
function epl_is_waitlist_flow() {
    ...
    if ( !epl_is_waitlist_record() && EPL_IS_ADMIN )
        return false;
    if ( get_the_regis_status( null, true ) == 5 ) {
        return false;
    }
...
function epl_mailchimp_subscribe() {
    ...
    if ( $sign_up == 1 && (epl_get_element_m( 'newsletter_signup', $erm->regis_id, $erm->current_data ) == 10) ) {
        $go = true;
    }
    elseif ( $sign_up == 3 )
        $go = true;
